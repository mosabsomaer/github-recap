...
let minCircleSize = 4;  // Increased for bigger boxes in text areas
let maxAddedCircleSize = 16;
let powerFactor = 2.5;  // Lower = bigger boxes in dark (text) areas

let boundaryCircleRadius = 500;
let canvasSize = 1000;
...
function onMouseDown(e) {
    ...
    let scaleX = canvasSize / canvasRect.width;
    let scaleY = canvasSize / canvasRect.height;

    let x = (e.clientX - canvasRect.x) * scaleX;
    let y = canvasSize - (e.clientY - canvasRect.y) * scaleY;
    x -= canvasSize / 2;
    y -= canvasSize / 2;
...
class Circle {
    ...
    draw() {
        ...
        // Draw square
        let size1 = this.r * 2;
        ctx.strokeRect(this.x - size1/2, this.y - size1/2, size1, size1);
    }
} 
...
vec3 calculateBoundingPushAwayVector(vec3 circle) {
    float boundary = ` + boundaryCircleRadius.toFixed(2) + `;
    vec3 pushAwayVec = vec3(0.0);

    // Check right boundary
    if(circle.x + circle.z > boundary) {
        pushAwayVec.x -= (circle.x + circle.z - boundary) * ` + repulsionFactor.toFixed(5) + `;
    }
    // Check left boundary
    if(circle.x - circle.z < -boundary) {
        pushAwayVec.x -= (circle.x - circle.z + boundary) * ` + repulsionFactor.toFixed(5) + `;
    ...
    // Check top boundary
    if(circle.y + circle.z > boundary) {
        pushAwayVec.y -= (circle.y + circle.z - boundary) * ` + repulsionFactor.toFixed(5) + `;
    }
